function StreetClaissification(  )

conf.calDir = 'data/StreetClass' ;
conf.dataDir = 'data/StreetClass' ;
...
conf.phowOpts = {'Step', 3,'ContrastThreshold', 0.10} ;
conf.clobber = false ;
...
%% Classify Video
if exist('data/StreetClass/tiny-model.mat','file')

load('data/StreetClass/tiny-model.mat')

    disp('Loading Video, May take a while');     
     
    vidObj = VideoReader('/media/diskAshur-DT/cctv/mjpeg/308 St Mary St-20121028-012648.avi')
    ...
    x = 1:numFrames;
    y = zeros(1,numFrames);
    for f = 1 :100: numFrames
       % disp(f/numFrames);
       im = read(vidObj,f);
       label = model.classify(model, im);
       %label = '012 Wood Street Pos 2'
